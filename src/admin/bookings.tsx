...
// Helper: format a Date as YYYY-MM-DD in the user's local time
...
export default function AdminBookings() {
  ...
  const [newService, setNewService] = useState<{ name: string; duration: number }>({
    name: '',
    duration: 30,
  });
  ...
  const addService = () => {
    ...
    setServices((prev) => [
      ...prev,
      { id: Math.random().toString(36).slice(2), name, duration },
    ]);
  ...
  // Weekday labels in the current i18n language
  const weekDays = Array.from({ length: 7 }, (_, i) =>
    new Intl.DateTimeFormat(i18n.language, { weekday: 'short' }).format(
      new Date(2021, 7, i + 1) // any fixed week works
    )
  );

  // Leading blanks (Sunday=0 ... Saturday=6). If you prefer Monday-first, adjust this value.
                ...
                <CardContent>
                  <div className="flex space-x-2 mb-4">
                    ...
                    <Input
                      ...
                      value={Number.isFinite(newService.duration) ? newService.duration : ''}
                      onChange={(e) =>
                        setNewService({
                          ...newService,
